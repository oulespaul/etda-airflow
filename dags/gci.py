...
def transform():
    ...
    df.drop('Code GCR', axis=1, inplace=True)
    ...
    df["pillar"] = df["GLOBAL ID"].apply(get_value_from_key, args=("pillar",))
    df["sub_pillar"] = df["GLOBAL ID"].apply(
        get_value_from_key, args=("sub_pillar",))
    df["sub_sub_pillar"] = df["GLOBAL ID"].apply(
        get_value_from_key, args=("sub_pillar_pillar",))
    df["indicator"] = df["GLOBAL ID"].apply(
        get_value_from_key, args=("indicator",))
    df["sub_index"] = df["GLOBAL ID"].apply(
        get_value_from_key, args=("sub_index",))
    df["sub_indicator"] = df["GLOBAL ID"].apply(
        get_value_from_key, args=("sub_indicator",))
    df["others"] = df["GLOBAL ID"].apply(get_value_from_key, args=("others",))

    df.drop('GLOBAL ID', axis=1, inplace=True)

    df = df.melt(
        id_vars=['Dataset', 'year', 'index_name', 'sub_index', 'pillar', 'sub_pillar',
    ...
    for year in year_list:
        ...
        final.to_csv('/opt/airflow/dags/output/gci/GCI_{}_{}.csv'.format(
...
def store_to_hdfs():
    ...
    my_dir = '/user/cloudera/raw/index_dashboard/Global/GCI'
...
with dag:
    ...
    clean_up_output = BashOperator(
        ...
        bash_command='rm -rf /opt/airflow/dags/output/gci/*',
    )
